...
parser.add_argument("--M", type=int, required=False, default=2048)
parser.add_argument("--N", type=int, required=False, default=2048)
parser.add_argument("--K", type=int, required=False, default=512)
parser.add_argument("--K_range", type=int, nargs=2)
parser.add_argument("--K_step", type=int, default=512)
parser.add_argument("--prec", type=str, choices=["fp8", "fp16"], default="fp16")
parser.add_argument("--kernel_run_once", type=str, choices=["naive", "persistent", "tma", "descriptor"], 
                  help="Run specific kernel implementation once")
parser.add_argument("--config_idx", type=int, help="Use specific configuration index for kernels")
...
def matmul_get_configs():
    ...
    if args.config_idx is not None:
        try:
            if not (0 <= args.config_idx < len(configs)):
                raise ValueError(f"config_idx must be between 0 and {len(configs)-1}")
            print(f"matmul_get_configs: picked index {configs[args.config_idx]}")
            return [configs[args.config_idx]]
        except ValueError as e:
            last_idx = len(configs) - 1
            print(f"Error: {str(e)}. Using last available config (index {last_idx}) instead.")
            return [configs[last_idx]]
    return configs
...
def matmul_tma_persistent_get_configs():
    configs = [
        triton.Config({'BLOCK_SIZE_M': BM, 'BLOCK_SIZE_N': BN, "BLOCK_SIZE_K" : BK, "GROUP_SIZE_M" : GS, "EPILOGUE_SUBTILE" : SUBTILE}, num_stages=s, num_warps=w) \
        for BM in [64,128,256] \
        for BN in [64,128,256] \
        for BK in [64,128,256] \
        for GS in [10,12,14] \
    ...
    if args.config_idx is not None:
        try:
            if not (0 <= args.config_idx < len(configs)):
                raise ValueError(f"config_idx must be between 0 and {len(configs)-1}")
            print(f"matmul_tma_persistent_get_configs: picked index {configs[args.config_idx]}")
            return [configs[args.config_idx]]
        except ValueError as e:
            last_idx = len(configs) - 1
            print(f"Error: {str(e)}. Using last available config (index {last_idx}) instead.")
...
def run_once(a, b, kernel_type):
    """Run the specified kernel implementation once.
    
    Args:
        a: Input matrix A
        b: Input matrix B
        kernel_type: String specifying which kernel to run ('naive', 'persistent', 'tma', 'descriptor')
    ...
    if kernel_type == 'naive':
        result = matmul(a, b.T)
        print(f"Best config index for naive kernel: {matmul_kernel.best_config_index}")
        return result
    elif kernel_type == 'persistent':
        result = matmul_persistent(a, b.T)
        print(f"Best config index for persistent kernel: {matmul_kernel_persistent.best_config_index}")
        return result
    elif kernel_type == 'tma':
        if not supports_tma():
            raise RuntimeError("TMA not supported on this device")
        result = matmul_tma_persistent(a, b)
        print(f"Best config index for TMA persistent kernel: {matmul_kernel_tma_persistent.best_config_index}")
        return result
    elif kernel_type == 'descriptor':
        if not supports_tma():
            raise RuntimeError("Tensor descriptors not supported on this device")
        result = matmul_descriptor_persistent(a, b)
        print(f"Best config index for descriptor persistent kernel: {matmul_kernel_descriptor_persistent.best_config_index}")
        return result
    else:
...
if __name__ == "__main__":
    if args.prec == 'fp8' and (not hasattr(torch, "float8_e4m3fn") or not is_cuda()):
        ...
        if args.kernel_run_once:
            # Run specific kernel once
            M, N, K = args.M, args.N, args.K_range[0]
            a = torch.randn((M, K), device="cuda", dtype=torch.float16).to(dtype)
            b = torch.randn((K, N), device="cuda", dtype=torch.float16).to(dtype)
            b = b.T.contiguous()
            
            try:
                result = run_once(a, b, args.kernel_run_once)
                print(f"Successfully ran {args.kernel_run_once} kernel")
