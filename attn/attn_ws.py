...
try:
    ...
    HAS_TRITONPARSE = True
except ImportError:
    HAS_TRITONPARSE = False
    print("Warning: TritonParse not available. Install with: pip install tritonparse")
...
ws_configs = [
    triton.Config({'BLOCK_M': 128, 'BLOCK_N': 128, 'NUM_BUFFERS': 2, 'NUM_MMA_WARPS': 4, 'NUM_MMA_GROUPS': 1},
                  num_stages=0, num_warps=4, pre_hook=_host_descriptor_pre_hook_ws),
    triton.Config({'BLOCK_M': 128, 'BLOCK_N': 128, 'NUM_BUFFERS': 2, 'NUM_MMA_WARPS': 8, 'NUM_MMA_GROUPS': 2},
                  num_stages=0, num_warps=4, pre_hook=_host_descriptor_pre_hook_ws),
    triton.Config({'BLOCK_M': 64, 'BLOCK_N': 64, 'NUM_BUFFERS': 2, 'NUM_MMA_WARPS': 4, 'NUM_MMA_GROUPS': 1},
                  num_stages=0, num_warps=4, pre_hook=_host_descriptor_pre_hook_ws),
]

import triton.tlx.language as tlx

@triton.autotune(configs=ws_configs, key=["N_CTX", "HEAD_DIM", "FP8_OUTPUT"])
@triton.jit
...
for mode in ["fwd"]:
    for causal in [False]:
        for warp_specialize in [False]: # if is_blackwell() else [False]:
            configs.append(
                triton.testing.Benchmark(
                    ...
                    line_names=["Triton [FP16]"] 
                            ...
                            # +(["Sage"] if HAS_SAGE else []),
                    # plot_type="bar",
                    styles=[("red", "-"), ("blue", "-"), ("green", "-"), ("orange", "-"), ("purple", "-"), ("yellow", "-"), ("pink", "-")],
                    ylabel="TFLOPS",
                    # ylabel="Time (ms)",
...
def bench_flash_attention(BATCH, H, N_CTX, HEAD_DIM, causal, warp_specialize, mode, provider, device=DEVICE):
    ...
    if HAS_PROTON:
        proton_scope_name = f"{provider}_{mode}_B{BATCH}_H{H}_N{N_CTX}_D{HEAD_DIM}"
        proton.enter_scope(proton_scope_name)
    ...
    if "triton" in provider:
        ...
        if HAS_PROTON:
            proton_metrics = {
                "flops": total_flops,
                "bytes": (q.numel() + k.numel() + v.numel()) * q.element_size(),
            }
            with proton.scope(f"{provider}_kernel", metrics=proton_metrics):
    ...
    # Exit proton scope and return results
    if HAS_PROTON:
        proton.exit_scope()
...
# Command line arguments for profiling
argparser = argparse.ArgumentParser()
argparser.add_argument("--profile", action="store_true", help="Enable proton profiling")
argparser.add_argument("--cudagraph", action="store_true", default=False, help="Use CUDA graphs for benchmarking")
argparser.add_argument("--tritonparse", action="store_true", help="Enable TritonParse tracing and analysis")
argparser.add_argument("--tritonparse-logs", type=str, default="./tritonparse_logs", help="Directory for TritonParse logs")
argparser.add_argument("--tritonparse-output", type=str, default="./tritonparse_output", help="Directory for TritonParse parsed output")
args = argparser.parse_args()


if __name__ == "__main__":
    # only works on post-Ampere GPUs right now
    if args.profile and HAS_PROTON:
        print("Proton profiling enabled")
        proton.start("flash_attention", hook="triton")
        bench_flash_attention.run(save_path=".", print_data=True)
        proton.finalize()
        print("Proton profiling complete. View results with: proton-viewer -m tflop/s,time/s ./flash_attention.hatchet")
    elif args.tritonparse and HAS_TRITONPARSE:
        print("TritonParse tracing enabled")
        
        # === TritonParse init ===
        # Ensure log directory exists
        os.makedirs(args.tritonparse_logs, exist_ok=True)
        os.makedirs(args.tritonparse_output, exist_ok=True)
        
        # Initialize structured logging to capture Triton compilation events
        print(f"Initializing TritonParse logging to: {args.tritonparse_logs}")
        tritonparse.structured_logging.init(args.tritonparse_logs)
        # === TritonParse init end ===
        
        # Disable FX graph cache to ensure all kernels are compiled
        os.environ["TORCHINDUCTOR_FX_GRAPH_CACHE"] = "0"
        
        # Run the benchmark to generate traces
        print("Running benchmark with TritonParse tracing...")
        bench_flash_attention.run(save_path=".", print_data=True)
        
        # === TritonParse parse ===
        print(f"Parsing TritonParse logs to: {args.tritonparse_output}")
        tritonparse.utils.unified_parse(source=args.tritonparse_logs, out=args.tritonparse_output)
        # === TritonParse parse end ===
        
        print("\n" + "="*80)
        print("TritonParse Analysis Complete!")
        print("="*80)
        print(f"Raw logs: {args.tritonparse_logs}")
        print(f"Parsed output: {args.tritonparse_output}")
        print("\nTo analyze results:")
        print("1. Visit: https://pytorch-labs.github.io/tritonparse/")
        print("2. Upload trace files from the parsed output directory")
        print("3. Explore using Overview and Code Comparison tabs")
        print("="*80)
    else:
        bench_flash_attention.run(save_path=".", print_data=True)